...
from torchao.testing.model_architectures import Llama4Experts
    ...
    def _test_moe_weight_reshape_ops(self, config):
        ...
        def _quantize_experts(model, config):
            for _, module in model.named_modules():
                if not isinstance(module, Llama4Experts):
                    continue
        ...
        moe1 = Llama4Experts(num_experts, dim, hidden_dim, dtype, device)
        moe2 = Llama4Experts(num_experts, dim, hidden_dim, dtype, device)
        moe_combined = Llama4Experts(num_experts, dim, 2 * hidden_dim, dtype, device)
